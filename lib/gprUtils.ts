...
/**
 * Светофор ГПР по отклонению готовности (п.п., факт − план) на отчётную дату.
 *
 * Пороги визуальной классификации:
 * - зелёный — выполнение в срок или впереди плана: deltaPp ≥ 0;
 * - жёлтый — умеренное отставание: −15 ≤ deltaPp &lt; 0 (до 15 п.п. к плану);
 * - красный — критическое отставание: deltaPp &lt; −15.
 *
 * Прежние пороги (≥0 / −10..0 / &lt;−10) приводили к раннему «красному» уже при
 * обычной просадке хода работ; новые пороги резервируют красный для действительно
 * критического отставания, а умеренная просадка отображается жёлтым.
 *
 * Замечания:
 * - функция меняет ТОЛЬКО цветовую классификацию; расчёт отклонения, плановых/факт
 *   процентов и дат не затрагивается;
 * - технический норматив платформы {@link GPR_PROGRESS_DELTA_CRITICAL_PP} (10 п.п.),
 *   используемый для сигналов риска в зависимостях ТМЦ, оставлен без изменений и
 *   намеренно строже визуального порога — это два разных контура (риск vs цвет).
 */
export function getStatusByGprProgressDelta(deltaPp: number): "green" | "yellow" | "red" {
  if (deltaPp >= 0) return "green";
  if (deltaPp >= -15) return "yellow";
  return "red";
}
