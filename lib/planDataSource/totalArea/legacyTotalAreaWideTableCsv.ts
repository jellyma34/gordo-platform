...
export type TotalAreaHeaderMap = {
  segment: string;
  planProject: string;
  planMonth: string;
...
const TOTAL_AREA_COLUMN_ALIASES = {
...
function isTotalAreaBenchmarkHeader(h: HeaderRow): boolean {
  ...
  return matchesAlias(h, TOTAL_AREA_COLUMN_ALIASES.planProject);
}

function isPlanProjectHeader(h: HeaderRow): boolean {
  if (isTotalAreaBenchmarkHeader(h)) return true;
  const c = h.compact;
...
export function projectColumnKindFromHeader(header: string): TotalAreaProjectColumnKind {
  const norm = normalizeApartmentPlanHeader(header);
  if (norm.includes("объект") && norm.includes("площад")) return "object_area";
  if (norm.includes("общ") && norm.includes("площад")) return "total_area";
...
export function totalAreaProjectColumnCaption(kind: TotalAreaProjectColumnKind): string {
  switch (kind) {
    case "object_area":
...
export function scoreTotalAreaWideTableColumns(metaFields: string[]): {
  hasName: boolean;
  ...
  const hasName = headers.some((h) => isNameHeader(h) || matchesAlias(h, TOTAL_AREA_COLUMN_ALIASES.name));
  const hasBenchmark = headers.some(
    (h) => isTotalAreaBenchmarkHeader(h) || isPlanProjectHeader(h) || matchesAlias(h, TOTAL_AREA_COLUMN_ALIASES.planProject),
  );
  const hasPlanMonth = headers.some(
    (h) => isPlanMonthHeader(h) || matchesAlias(h, TOTAL_AREA_COLUMN_ALIASES.planMonth),
  );
  const hasPlanCum = headers.some(
    (h) => isPlanCumulativeHeader(h) || matchesAlias(h, TOTAL_AREA_COLUMN_ALIASES.planCumulative),
  );
...
export function detectTotalAreaWideTableCsv(metaFields: string[]): boolean {
  if (detectLegacyWideTableCsv(metaFields)) return true;
  const { hasName, hasPlanCum, hasBenchmark, matched } = scoreTotalAreaWideTableColumns(metaFields);
  return hasName && hasPlanCum && hasBenchmark && matched >= 3;
}

function resolveTotalAreaFactHeaders(metaFields: string[]): { factMonth: string; factCumulative: string } {
  const headers = headerRows(metaFields);
  ...
  return {
    factMonth: pick(isFactMonthHeader, TOTAL_AREA_COLUMN_ALIASES.factMonth),
    factCumulative: pick(isFactCumulativeHeader, TOTAL_AREA_COLUMN_ALIASES.factCumulative),
  };
}

export function resolveTotalAreaWideHeaders(
  metaFields: string[],
): { map: TotalAreaHeaderMap; projectColumnKind: TotalAreaProjectColumnKind } | { error: string } {
  if (detectLegacyWideTableCsv(metaFields)) {
    const plan = resolveColumnarPlanHeaders(metaFields);
    if ("error" in plan) return plan;
    const fact = resolveTotalAreaFactHeaders(metaFields);
    return {
      map: {
  ...
  const segment = pick(isNameHeader, TOTAL_AREA_COLUMN_ALIASES.name);
  const planProject = pick(isPlanProjectHeader, TOTAL_AREA_COLUMN_ALIASES.planProject);
  const planMonth = pick(isPlanMonthHeader, TOTAL_AREA_COLUMN_ALIASES.planMonth);
  const planCumulative = pick(isPlanCumulativeHeader, TOTAL_AREA_COLUMN_ALIASES.planCumulative);
  const factMonth = pick(isFactMonthHeader, TOTAL_AREA_COLUMN_ALIASES.factMonth);
  const factCumulative = pick(isFactCumulativeHeader, TOTAL_AREA_COLUMN_ALIASES.factCumulative);

  if (!segment || !planProject || !planMonth || !planCumulative) {
    return {
      error: `Не найдены колонки («Наименование», «Общая площадь» / «Площадь объекта», план на отчётный месяц, план накопит. итогом). Заголовки: ${headers.map((h) => h.original).join(" · ")}`,
    };
  }
...
export function totalAreaColumnMappingForDiagnostics(map: TotalAreaHeaderMap): Record<string, string> {
  return {
    segment: map.segment,
...
export function totalAreaCsvTypeLabel(
  metaFields: string[],
): "legacy_wide_table" | "total_area_wide_table" | "unknown" {
  if (detectLegacyWideTableCsv(metaFields)) return "legacy_wide_table";
  if (detectTotalAreaWideTableCsv(metaFields)) return "total_area_wide_table";
  return "unknown";
}
