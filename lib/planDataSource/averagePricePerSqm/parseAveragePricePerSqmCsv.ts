...
import {
  averagePriceColumnMappingForDiagnostics,
  averagePriceCsvTypeLabel,
  resolveAveragePriceWideHeaders,
} from "@/lib/planDataSource/averagePricePerSqm/legacyAveragePriceWideTableCsv";
import type {
  AveragePricePerSqmCsvParseResult,
  AveragePricePerSqmEntitySummary,
  AveragePricePerSqmNormalizedRow,
  AveragePriceProjectColumnKind,
  ParseAveragePricePerSqmCsvOptions,
...
export const AVERAGE_PRICE_PER_SQM_CSV_MAX_BYTES = 10 * 1024 * 1024;
...
function isAveragePriceRoomTypeRow(segmentNorm: string, rawLabel: string): boolean {
...
function parseAveragePriceGrid(
  ...
  const out: AveragePricePerSqmNormalizedRow[] = [];
  let apartmentsSummary: AveragePricePerSqmEntitySummary | null = null;
  let parkingSummary: AveragePricePerSqmEntitySummary | null = null;
  let storageSummary: AveragePricePerSqmEntitySummary | null = null;
  ...
  for (let i = 0; i < rowsIn.length; i++) {
    ...
    if (
      ...
      !isAveragePriceRoomTypeRow(segmentNorm, rawLabel) &&
      !isAveragePriceEntityDetailRow(segmentNorm, rawLabel)
    ) {
      continue;
    }
    ...
    importedRootRows += 1;
    const row: AveragePricePerSqmNormalizedRow = {
      segmentNorm,
      monthKey,
  ...
  if (!out.length) {
    return {
      ...
      diagnostics: {
        rawHeaders: metaFields,
        columnMapping: averagePriceColumnMappingForDiagnostics(map),
        delimiter: null,
        csvType: averagePriceCsvTypeLabel(metaFields),
...
export function parseAveragePricePerSqmCsvAsync(
  text: string,
  options?: ParseAveragePricePerSqmCsvOptions,
): Promise<AveragePricePerSqmCsvParseResult> {
  ...
  try {
    ...
    const csvType = averagePriceCsvTypeLabel(rawHeaders);
    if (csvType === "unknown") {
      return Promise.resolve({
        ok: false,
        error:
          "Не распознан формат CSV. Ожидается таблица: «Наименование», «Средняя стоимость объекта» / «Средняя стоимость м²», план/факт на отчётный период.",
        warnings: [],
        diagnostics: {
          rawHeaders,
          columnMapping: null,
    ...
    const headerPick = resolveAveragePriceWideHeaders(rawHeaders);
    const projectColumnKind = "error" in headerPick ? "plan_project" : headerPick.projectColumnKind;

    const parsed = parseAveragePriceGrid(rawHeaders, grid.rows, monthKey, projectColumnKind);
    if (!parsed.ok) {
      return Promise.resolve({
        ...parsed,
