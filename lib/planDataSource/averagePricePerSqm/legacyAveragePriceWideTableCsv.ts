...
export type AveragePricePerSqmHeaderMap = {
  segment: string;
  planProject: string;
  planMonth: string;
...
const AVG_PRICE_COLUMN_ALIASES = {
...
function isAvgPriceBenchmarkHeader(h: HeaderRow): boolean {
  if (h.norm.includes("средн") && (h.norm.includes("стоим") || h.norm.includes("цен"))) {
    if (h.norm.includes("объект") || h.norm.includes("м2") || h.norm.includes("м²") || h.norm.includes("кв")) {
      return true;
    }
  }
  return matchesAlias(h, AVG_PRICE_COLUMN_ALIASES.planProject);
}

function isPlanProjectHeader(h: HeaderRow): boolean {
  if (isAvgPriceBenchmarkHeader(h)) return true;
  const c = h.compact;
  if (c.startsWith("планпроект") || c === compactCsvHeader("план проекта")) return true;
  return (
    h.norm.includes("план") &&
...
export function projectColumnKindFromHeader(header: string): AveragePriceProjectColumnKind {
  const norm = normalizeApartmentPlanHeader(header);
  if (norm.includes("объект")) return "object";
  if (norm.includes("м2") || norm.includes("м²") || norm.includes("кв")) return "sqm";
  return "plan_project";
}

export function averagePriceProjectColumnCaption(kind: AveragePriceProjectColumnKind): string {
  switch (kind) {
    case "object":
      return "Средняя стоимость объекта";
    case "sqm":
      return "Средняя стоимость м²";
    default:
      return "План проекта";
  }
}

export function scoreAveragePriceWideTableColumns(metaFields: string[]): {
  hasName: boolean;
  ...
  const hasName = headers.some((h) => isNameHeader(h) || matchesAlias(h, AVG_PRICE_COLUMN_ALIASES.name));
  const hasBenchmark = headers.some(
    (h) => isAvgPriceBenchmarkHeader(h) || isPlanProjectHeader(h) || matchesAlias(h, AVG_PRICE_COLUMN_ALIASES.planProject),
  );
  const hasPlanMonth = headers.some(
    (h) => isPlanMonthHeader(h) || matchesAlias(h, AVG_PRICE_COLUMN_ALIASES.planMonth),
  );
  const hasPlanCum = headers.some(
    (h) => isPlanCumulativeHeader(h) || matchesAlias(h, AVG_PRICE_COLUMN_ALIASES.planCumulative),
  );
...
export function detectAveragePriceWideTableCsv(metaFields: string[]): boolean {
  if (detectLegacyWideTableCsv(metaFields)) return true;
  const { hasName, hasPlanCum, hasBenchmark, matched } = scoreAveragePriceWideTableColumns(metaFields);
  return hasName && hasPlanCum && hasBenchmark && matched >= 3;
}

function resolveAveragePriceFactHeaders(metaFields: string[]): { factMonth: string; factCumulative: string } {
  const headers = headerRows(metaFields);
  ...
  return {
    factMonth: pick(isFactMonthHeader, AVG_PRICE_COLUMN_ALIASES.factMonth),
    factCumulative: pick(isFactCumulativeHeader, AVG_PRICE_COLUMN_ALIASES.factCumulative),
  };
}

export function resolveAveragePriceWideHeaders(
  metaFields: string[],
): { map: AveragePricePerSqmHeaderMap; projectColumnKind: AveragePriceProjectColumnKind } | { error: string } {
  if (detectLegacyWideTableCsv(metaFields)) {
    const plan = resolveColumnarPlanHeaders(metaFields);
    if ("error" in plan) return plan;
    const fact = resolveAveragePriceFactHeaders(metaFields);
    return {
      map: {
  ...
  const segment = pick(isNameHeader, AVG_PRICE_COLUMN_ALIASES.name);
  const planProject = pick(isPlanProjectHeader, AVG_PRICE_COLUMN_ALIASES.planProject);
  const planMonth = pick(isPlanMonthHeader, AVG_PRICE_COLUMN_ALIASES.planMonth);
  const planCumulative = pick(isPlanCumulativeHeader, AVG_PRICE_COLUMN_ALIASES.planCumulative);
  const factMonth = pick(isFactMonthHeader, AVG_PRICE_COLUMN_ALIASES.factMonth);
  const factCumulative = pick(isFactCumulativeHeader, AVG_PRICE_COLUMN_ALIASES.factCumulative);

  if (!segment || !planProject || !planMonth || !planCumulative) {
    return {
      error: `Не найдены колонки («Наименование», «Средняя стоимость» / «План проекта», план на отчётный месяц, план накопит. итогом). Заголовки: ${headers.map((h) => h.original).join(" · ")}`,
    };
  }
...
export function averagePriceColumnMappingForDiagnostics(map: AveragePricePerSqmHeaderMap): Record<string, string> {
  return {
    segment: map.segment,
...
export function averagePriceCsvTypeLabel(
  metaFields: string[],
): "legacy_wide_table" | "average_price_wide_table" | "unknown" {
  if (detectLegacyWideTableCsv(metaFields)) return "legacy_wide_table";
  if (detectAveragePriceWideTableCsv(metaFields)) return "average_price_wide_table";
  return "unknown";
}
