...
export type CashflowSeriesRow = {
  ...
  planMonth: number;
  /** Факт поступлений за месяц, ₽ — только из явных колонок притока CSV; иначе `null`. */
  factMonth: number | null;
};

/**
 * Поступления: **план** из колонок графика платежей («План <месяц>»), **факт** — только из колонок
 * с явной семантикой притока («Факт …», «Оплачено …», «зайдет …» в Верба), сумма по строкам сделок в столбце.
 * Сальдо, накопительные итоги и долг в факт не подставляются.
 */
