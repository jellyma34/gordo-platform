...
export async function createUserRequest(token: string, body: CreateUserPayload) {
  const res = await fetchAuthorizedApi("/api/admin/users", token, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function updateAdminUser(token: string, userId: number, body: UpdateUserPayload): Promise<AdminUserRow> {
  const res = await fetchAuthorizedApi(`/api/admin/users/${userId}`, token, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function blockAdminUser(token: string, userId: number, reason?: string): Promise<AdminUserRow> {
  const res = await fetchAuthorizedApi(`/api/admin/users/${userId}/block`, token, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function unblockAdminUser(token: string, userId: number): Promise<AdminUserRow> {
  const res = await fetchAuthorizedApi(`/api/admin/users/${userId}/unblock`, token, {
    method: "PUT",
  });
...
export async function getAdminUserAnalytics(token: string, userId: number): Promise<UserAnalytics> {
  const res = await fetchAuthorizedApi(`/api/admin/users/${userId}/analytics`, token, {});
...
export async function listEntityVersions(token: string, entityId: number): Promise<EntityVersionListItem[]> {
  const res = await fetchAuthorizedApi(`/api/entity/${entityId}/versions`, token, {});
...
export async function getEntityVersion(
  ...
  const res = await fetchAuthorizedApi(`/api/entity/${entityId}/versions/${versionId}`, token, {});
...
export async function rollbackEntityVersion(
  ...
  const res = await fetchAuthorizedApi(`/api/entity/${entityId}/rollback/${versionId}?type=${entityType}`, token, {
    method: "POST",
  });
...
export async function listEntityHistory(
  ...
  const res = await fetchAuthorizedApi(`/api/entity/${entityId}/history?type=${entityType}`, token, {});
...
export async function getEntityHistoryItem(
  ...
  const res = await fetchAuthorizedApi(`/api/entity/${entityId}/history/${historyId}?type=${entityType}`, token, {});
...
export async function setAdminUserPassword(token: string, userId: number, password: string): Promise<void> {
  const res = await fetchAuthorizedApi(`/api/admin/users/${userId}/password`, token, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function listActivityLogs(
  ...
  const res = await fetchAuthorizedApi(`/api/admin/logs?${q.toString()}`, token, {});
  if (!res.ok) await adminJsonError(res, "Не удалось загрузить историю");
...
export async function getRelatedDeviations(token: string, taskId: number): Promise<RelatedDeviationRow[]> {
  const res = await fetchAuthorizedApi(`/api/gpr/tasks/${taskId}/related-deviations`, token, {});
...
export async function listGprTasksApi(token: string, partId?: number): Promise<GprTaskApiItem[]> {
  ...
  const res = await fetchAuthorizedApi(`/api/gpr/tasks${q}`, token, {});
...
export async function updateGprTaskApi(
  ...
  const res = await fetchAuthorizedApi(`/api/gpr/tasks/${taskId}`, token, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function createGprTaskApi(token: string, body: GprTaskWritePayload): Promise<GprTaskApiItem> {
  const res = await fetchAuthorizedApi(`/api/gpr/tasks`, token, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
export async function getEntityGprTaskApi(token: string, entityId: number): Promise<GprTaskApiItem> {
  const res = await fetchAuthorizedApi(`/api/entity/${entityId}`, token, {});
