...
/** Разбор CSV машино-мест (Excel RU): `;` или `,`, UTF-8 / cp1251 на стороне загрузки. */

export type ParseParkingCsvOk = {
  ok: true;
  rows: string[][];
  headers: string[];
  uploadedAt: string;
...
export type ParseParkingCsvFail = {
  ok: false;
  error: string;
  warnings?: string[];
};

export type ParseParkingCsvResult = ParseParkingCsvOk | ParseParkingCsvFail;
...
function isSkippableParkingRow(row: string[]): boolean {
...
/** Строка заголовков реестра машино-мест. */
function isParkingRegistryHeaderLine(line: string): boolean {
  ...
  return (
    ...
    n.includes("parking") ||
    (n.includes("стоим") && (n.includes("мм") || n.includes("мест")))
  );
}

/**
 * Прочитать CSV машино-мест: заголовки + строки данных (без строки заголовка в `rows`).
 */
export function parseParkingCsv(text: string, filename = "upload.csv"): ParseParkingCsvResult {
  ...
  for (let i = 0; i < nonEmpty.length; i++) {
    if (isParkingRegistryHeaderLine(nonEmpty[i] ?? "")) {
      headerLineIdx = i;
      break;
    }
  }
  ...
  const rows = table.slice(headerLineIdx + 1).filter((row) => {
    if (isSkippableParkingRow(row)) return false;
