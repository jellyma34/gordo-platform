...
function filePath(projectId: string): string {
  ...
  return path.join(dir, `gpr-import-${safeProjectSegment(projectId)}.json`);
}

/** GET — последний сохранённый снимок задач ГПР для проекта. */
export async function GET(req: NextRequest) {
  const projectId = req.nextUrl.searchParams.get("projectId") ?? "default";
  try {
    const raw = await readFile(filePath(projectId), "utf-8");
    const parsed = JSON.parse(raw) as { tasks?: unknown; updatedAt?: string };
    if (!parsed || !Array.isArray(parsed.tasks)) {
      return NextResponse.json({ tasks: [], updatedAt: null }, { status: 200 });
    }
...
/** POST — сохранить снимок (импорт CSV / массовая замена). */
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("[api/gpr/import] POST failed:", e);
