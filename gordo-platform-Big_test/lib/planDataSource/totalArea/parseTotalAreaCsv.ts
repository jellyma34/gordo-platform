...
import {
  resolveTotalAreaWideHeaders,
  totalAreaColumnMappingForDiagnostics,
  totalAreaCsvTypeLabel,
} from "@/lib/planDataSource/totalArea/legacyTotalAreaWideTableCsv";
import type {
  ParseTotalAreaCsvOptions,
  TotalAreaCsvParseResult,
...
export const TOTAL_AREA_CSV_MAX_BYTES = 10 * 1024 * 1024;
...
function isTotalAreaRoomTypeRow(segmentNorm: string, rawLabel: string): boolean {
...
function parseTotalAreaGrid(
  ...
  const out: TotalAreaNormalizedRow[] = [];
  let apartmentsSummary: TotalAreaEntitySummary | null = null;
  let parkingSummary: TotalAreaEntitySummary | null = null;
  let storageSummary: TotalAreaEntitySummary | null = null;
  ...
  for (let i = 0; i < rowsIn.length; i++) {
    ...
    if (
      ...
      !isTotalAreaRoomTypeRow(segmentNorm, rawLabel) &&
      !isTotalAreaEntityDetailRow(segmentNorm, rawLabel)
    ) {
      continue;
    }
    ...
    importedRootRows += 1;
    const row: TotalAreaNormalizedRow = {
      segmentNorm,
      monthKey,
  ...
  if (!out.length) {
    return {
      ...
      diagnostics: {
        rawHeaders: metaFields,
        columnMapping: totalAreaColumnMappingForDiagnostics(map),
        delimiter: null,
        csvType: totalAreaCsvTypeLabel(metaFields),
...
export function parseTotalAreaCsvAsync(
  text: string,
  options?: ParseTotalAreaCsvOptions,
): Promise<TotalAreaCsvParseResult> {
  ...
  try {
    ...
    const csvType = totalAreaCsvTypeLabel(rawHeaders);
    if (csvType === "unknown") {
      return Promise.resolve({
        ok: false,
        error:
          "Не распознан формат CSV. Ожидается таблица: «Наименование», «Общая площадь» / «Площадь объекта», план/факт на отчётный период.",
        warnings: [],
        diagnostics: {
          rawHeaders,
          columnMapping: null,
    ...
    const headerPick = resolveTotalAreaWideHeaders(rawHeaders);
    const projectColumnKind = "error" in headerPick ? "plan_project" : headerPick.projectColumnKind;

    const parsed = parseTotalAreaGrid(rawHeaders, grid.rows, monthKey, projectColumnKind);
    if (!parsed.ok) {
      return Promise.resolve({
        ...parsed,
