...
export function EditLayout({
  ...
  showActions?: boolean;
  /** Заголовок и подзаголовок в шапке карточки */
  showTitle?: boolean;
}) {
  ...
  const showHeader = showTitle || hasActions;
  ...
  return (
    <div className="edit-layout w-full min-w-0 overflow-x-clip">
      <div className="edit-content min-w-0 rounded-xl border border-slate-200 bg-white shadow-sm">
        {showHeader ? (
          <div
            className={`edit-header flex border-b border-slate-100 px-5 md:px-6 ${
              showTitle
                ? "flex-col gap-4 py-5 md:flex-row md:items-start md:justify-between md:py-6"
                : "flex-row items-center justify-end gap-2 py-3 md:py-3"
            }`}
          >
            {showTitle ? (
