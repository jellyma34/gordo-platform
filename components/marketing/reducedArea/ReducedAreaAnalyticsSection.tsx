...
import { buildSalesPlanReducedAreaApartmentByRoomType } from "@/lib/reducedArea/buildSalesPlanReducedAreaApartmentByRoomType";
import { buildSalesPlanReducedAreaByObjectType } from "@/lib/reducedArea/buildSalesPlanReducedAreaByObjectType";
import { resolveReducedAreaActiveSlice } from "@/lib/reducedArea/resolveReducedAreaActiveSlice";
import { formatReducedAreaCompact } from "@/lib/reducedAreaPeriodKpi";
import {
  useReducedAreaBlock,
  type ReducedAreaBlockState,
  type UseReducedAreaBlockArgs,
} from "@/lib/reducedArea/useReducedAreaBlock";
import {
...
type Props = UseReducedAreaBlockArgs &
  ...
  className?: string;
  block?: ReducedAreaBlockState;
};

const SECTION_TITLE = "Приведенная площадь, кв.м";

export function ReducedAreaAnalyticsSection({
  presentation,
  presDark,
  mplPremium = false,
  showCsvUpload = false,
  ...
  const salesPlanByObjectType = useMemo(() => buildSalesPlanReducedAreaByObjectType({ doc: filteredDoc ?? null }), [filteredDoc]);
  const apartmentByRoomType = useMemo(
    () => buildSalesPlanReducedAreaApartmentByRoomType({ doc: filteredDoc ?? null }).byRoomType,
    [filteredDoc],
  );
  ...
  const kpiSectionProps = {
    ...
    mplPremium,
    embedded: true,
    formatMetric: formatReducedAreaCompact,
  ...
  return (
    <div
      className={`reduced-area-analytics w-full min-w-0 max-w-none ${className}`.trim()}
      onDragOver={
        showCsvUpload
          ? (e: DragEvent) => {
              e.preventDefault();
      ...
      >
        {showSkeleton ? (
          ...
          <AnalyticsKpiCard
            {...kpiSectionProps}
            showEmpty
            emptyMessage="Загрузите CSV приведенной площади или дождитесь данных"
          />
        ) : tabEmpty ? (
          <AnalyticsKpiCard
            {...kpiSectionProps}
