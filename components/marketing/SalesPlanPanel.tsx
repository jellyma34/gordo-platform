...
import {
  buildStoragePlanAnalyticsBreakdown,
  buildStorageTotals,
...
} from "@/lib/marketingCsvLocalMigration";
import {
...
import { SalesPlanExecutionBlock, MarketingExtendedAnalyticsAccordion } from "@/components/marketing/SalesPlanExecutionBlock";
import {
...
import { resolveInstallmentFactThroughPeriodKey } from "@/lib/buildInstallmentForecastChartData";
...
export function SalesPlanPanel({ presentation, period, objectId, initialPlanScenario, pdfMode = false }: Props) {
  ...
  /**
   * Единые условия для CSV в блоке «Исполнение плана продаж» (KPI квартир, лиды, plan_fact и т.д.):
   * не презентационный маршрут и (глобальный «Редактирование» или вкладка плана «Редактирование»).
   */
  const planChromeEditMode = !presentation && (appMode === "edit" || mode === "edit");
  ...
  const [schedulePlanMeta, setSchedulePlanMeta] = useState<MarketingPaymentPlanMeta | null>(null);
  const [scheduleFactMeta, setScheduleFactMeta] = useState<MarketingPaymentPlanMeta | null>(null);
  ...
  /** Не блокировать дашборд, если гидратация CSV/API зависла (битый .next, сеть). */
  useEffect(() => {
    const id = window.setTimeout(() => {
      setPaymentPlanHydrated((v) => {
        if (!v) console.warn("[marketing] paymentPlanHydrated timeout — показываем UI без ожидания");
        return true;
      });
      setExecutionHydrated((v) => {
        if (!v) console.warn("[marketing] executionHydrated timeout — показываем UI без ожидания");
        return true;
      });
      ...
      setMarketingLeadsHydrated((v) => (v ? v : true));
    }, 12_000);
  ...
  useEffect(() => {
    ...
    const applyServerPlan = (plan: MarketingPaymentPlanFileV2 | null | undefined) => {
      if (!plan || plan.v !== 2) {
        ...
        setSchedulePlanMeta(null);
        setScheduleFactMeta(null);
      ...
      setSchedulePlanMeta(plan.planMeta ?? null);
      setScheduleFactMeta(plan.factMeta ?? null);
    ...
    (async () => {
      try {
        ...
        const migrated = await migrateMarketingCsvFromLocalStorageIfNeeded({
          projectId: paymentPlanProjectId,
          uploadedBy: paymentUploadedByLabel,
        ...
        if (migrated.paymentPlan?.v === 2) {
          applyServerPlan(migrated.paymentPlan);
          setPaymentPlanHydrated(true);
          return;
        }
  ...
  const applyPaymentPlanResponse = (doc: MarketingPaymentPlanFileV2) => {
    const p = doc.planByPeriodKey;
    setPaymentPlanByPeriodKey(Object.keys(p).length > 0 ? p : null);
    const f = doc.factByPeriodKey;
    setPaymentFactByPeriodKey(f != null && Object.keys(f).length > 0 ? f : null);
    setPaymentFactUnavailableReason(doc.factUnavailableReason);
  ...
  const onPaymentPlanCsvSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!res.ok || !j?.ok) {
        setScheduleError(typeof j?.error === "string" ? j.error : "Не удалось сохранить файл плана на сервере.");
        return;
      }
      setScheduleWarnings(Array.isArray(j.warnings) && j.warnings.length > 0 ? j.warnings : null);
      if (j.plan?.v === 2) applyPaymentPlanResponse(j.plan);
    } catch {
      setScheduleError("Не удалось прочитать или отправить файл плана.");
    }
  };

  const onPaymentFactCsvSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    setScheduleError(null);
    ...
    try {
      ...
      if (!res.ok || !j?.ok) {
        setScheduleError(typeof j?.error === "string" ? j.error : "Не удалось сохранить файл факта на сервере.");
        return;
      }
      setScheduleWarnings(Array.isArray(j.warnings) && j.warnings.length > 0 ? j.warnings : null);
      if (j.plan?.v === 2) applyPaymentPlanResponse(j.plan);
    } catch {
      setScheduleError("Не удалось прочитать или отправить файл факта.");
    }
  };

  const clearPaymentPlanCsv = async () => {
    setScheduleError(null);
    setScheduleWarnings(null);
    try {
      const dr = await fetch(
        `/api/marketing/payment-plan?projectId=${encodeURIComponent(paymentPlanProjectId)}&scope=plan`,
        { method: "DELETE" },
      );
      if (!dr.ok) {
        setScheduleError("Не удалось сбросить план на сервере.");
        return;
      }
      const res = await fetch(
        `/api/marketing/payment-plan?projectId=${encodeURIComponent(paymentPlanProjectId)}`,
      ...
      const j = (await res.json()) as { plan?: MarketingPaymentPlanFileV2 | null };
      if (j?.plan?.v === 2) applyPaymentPlanResponse(j.plan);
      else {
        setPaymentPlanByPeriodKey(null);
        setSchedulePlanMeta(null);
      ...
      setScheduleError("Не удалось сбросить план.");
    }
    if (paymentPlanCsvInputRef.current) paymentPlanCsvInputRef.current.value = "";
  };

  const clearPaymentFactCsv = async () => {
    setScheduleError(null);
    setScheduleWarnings(null);
    try {
      const dr = await fetch(
        `/api/marketing/payment-plan?projectId=${encodeURIComponent(paymentPlanProjectId)}&scope=fact`,
        { method: "DELETE" },
      );
      if (!dr.ok) {
        setScheduleError("Не удалось сбросить факт на сервере.");
        return;
      }
      const res = await fetch(
        `/api/marketing/payment-plan?projectId=${encodeURIComponent(paymentPlanProjectId)}`,
      ...
      setScheduleError("Не удалось сбросить факт.");
    }
    if (paymentFactCsvInputRef.current) paymentFactCsvInputRef.current.value = "";
  };
  ...
  const cashflowPlanNote = hasAnyPaymentCsv
    ...
    : `Загрузите CSV графика платежей (план) и при необходимости отдельный CSV факта поступлений. Ось с ${salesStartLabel}.`;
  ...
  const installmentFactThroughPeriodKey = useMemo(
    () => resolveInstallmentFactThroughPeriodKey(analytics.currentPeriodKey, period),
    [analytics.currentPeriodKey, period],
  );
    ...
    try {
      ...
      const dealFacts =
        dealsFeed.loading && dealRowsForFact.length === 0
          ? { factMonth: 0, factCumulative: 0, debug: null as null }
          : apartmentPlanFactsFromDealsForKpi(dealRowsForFact, { period, currentPeriodKey: currentKey });
      ...
      const hasCsvPlan = Array.isArray(rows) && rows.length > 0;
      ...
      const apartmentTotals = buildApartmentTotals(typeBreakdown);

      if (process.env.NODE_ENV !== "production") {
        console.log({
          apartmentTotals,
          cumulativePlan: apartmentTotals.planCumulative,
          cumulativeFact: apartmentTotals.factCumulative,
          selectedMonth: currentKey,
        });
      }

      if (hasCsvPlan && apartmentPlanKpiDoc) {
        const planStrategy = getPlanCalculationStrategy(planCsvType);
        const apartmentsSummary = apartmentPlanKpiDoc.biReportMeta?.apartmentsSummary ?? null;
        ...
        const merged = mergeApartmentPlanCsvWithFacts(
          {
            planMonth: apartmentTotals.planMonth,
            planCumulative: apartmentTotals.planCumulative,
            totalVolume: 0,
            cumulativeMode: planStrategy.cumulativeMode,
          },
          {
            factMonth: apartmentTotals.factMonth,
            factCumulative: apartmentTotals.factCumulative,
          },
        ...
        return {
          ...
          planCalcDebug: {
            calculationStrategyLabel: planStrategy.calculationStrategyLabel,
            cumulativeDebugEn: planStrategy.cumulativeDebugEn,
            cumulativeDebugRu: planStrategy.cumulativeDebugRu,
            kpiEntity: "Apartments" as const,
            csvSummaryRow: "Σ 1к+2к+3к+4к+",
            selectedMonthLabel: currentKey,
            planCumulativeSource: apartmentTotals.planCumulative,
          },
          typeBreakdown,
        };
      }
  ...
  const parkingPlanBundle = useMemo(() => {
    try {
      ...
      const planCsvType =
        ...
        (apartmentPlanKpiDoc?.biReportMeta?.apartmentsSummary ? "bi_report" : undefined);
      const hasCsvPlan = Array.isArray(rows) && rows.length > 0;
      const breakdown = buildParkingPlanAnalyticsBreakdown({
        rows,
        hasCsvPlan,
        csvType: planCsvType,
        period,
      ...
      if (hasCsvPlan) {
        ...
        const planSlice = {
          planMonth: totals.planMonth,
          planCumulative: totals.planCumulative,
          totalVolume: slice?.totalVolume ?? 0,
          cumulativeMode: slice?.cumulativeMode ?? ("bi_report_ready_column" as const),
        };
        return {
          breakdown,
          periodKpi: {
            hasCsvPlan: true as const,
            ...mergeParkingPlanCsvWithFacts(planSlice, {
              factMonth: totals.factMonth,
              factCumulative: totals.factCumulative,
            }),
          },
        };
      }
      ...
      return {
        breakdown,
        periodKpi: { hasCsvPlan: false as const, factMonth: totals.factMonth, factCumulative: totals.factCumulative },
      };
    } catch (e) {
      console.error("parkingPlanBundle:", e);
      return {
        breakdown: { hasCsvPlan: false, items: [] },
  ...
  const storagePlanBundle = useMemo(() => {
    try {
      ...
      const planCsvType =
        ...
        (apartmentPlanKpiDoc?.biReportMeta?.apartmentsSummary ? "bi_report" : undefined);
      const hasCsvPlan = Array.isArray(rows) && rows.length > 0;
      const breakdown = buildStoragePlanAnalyticsBreakdown({
        rows,
        hasCsvPlan,
        csvType: planCsvType,
        period,
      ...
      if (hasCsvPlan) {
        ...
        return {
          ...
          periodKpi: {
            hasCsvPlan: true as const,
            ...mergeStoragePlanCsvWithFacts(planSlice, {
              factMonth: totals.factMonth,
              factCumulative: totals.factCumulative,
            }),
          },
      ...
      console.error("storagePlanBundle:", e);
      return {
        breakdown: { hasCsvPlan: false, items: [] },
        periodKpi: { hasCsvPlan: false as const, factMonth: 0, factCumulative: 0 },
      };
  ...
  const velocityLineData = useMemo(() => {
    try {
      if (!Array.isArray(monthlyPlanExecutionData) || monthlyPlanExecutionData.length === 0) return [];
      return buildVelocityLineRows(monthlyPlanExecutionData);
    } catch (e) {
      console.error("[SalesPlanPanel] velocityLineData", e);
      return [];
    }
  }, [monthlyPlanExecutionData]);
  ...
  if (pdfMode) {
    return (
      <SalesPlanMarketingPdfBlocks
        ...
        paymentFactByPeriodKey={hasFactMonths ? paymentFactByPeriodKey : null}
        installmentFactThroughPeriodKey={installmentFactThroughPeriodKey}
    ...
    <div className={MARKETING_DASHBOARD_SECTION_STACK_CLASS}>
      ...
      >
        <InstallmentForecastSection
          ...
          isEditMode={planChromeEditMode}
          period={period}
          factByPeriodKey={hasFactMonths ? paymentFactByPeriodKey : null}
          factThroughPeriodKey={installmentFactThroughPeriodKey}
        />
      </div>
        ...
        >
          <div className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <input
                ref={paymentFactCsvInputRef}
                type="file"
                accept=".csv,text/csv"
                className="hidden"
                onChange={onPaymentFactCsvSelected}
              />
              <button
                type="button"
                ...
                onClick={() => paymentFactCsvInputRef.current?.click()}
              >
                Загрузить факт поступлений (CSV)
              </button>
              {hasFactCsv ? (
                <button
                  type="button"
                  className="text-xs font-semibold text-rose-600 hover:text-rose-500"
                  onClick={() => void clearPaymentFactCsv()}
                >
                  Сбросить факт (CSV)
                </button>
              ) : null}
            ...
            <div className="flex flex-wrap items-center gap-3">
              <input
                ref={paymentPlanCsvInputRef}
                type="file"
                accept=".csv,text/csv"
                className="hidden"
                onChange={onPaymentPlanCsvSelected}
              />
              <button
                type="button"
                disabled={!paymentPlanHydrated}
                className={
                  presDark
                    ? "rounded-md border border-amber-400/40 bg-amber-500/15 px-2.5 py-1.5 text-xs font-semibold text-amber-200 hover:bg-amber-500/25 disabled:cursor-not-allowed disabled:opacity-45"
                    : "rounded-md border border-amber-500/50 bg-amber-500/10 px-2.5 py-1.5 text-xs font-semibold text-amber-800 hover:bg-amber-500/15 disabled:cursor-not-allowed disabled:opacity-45"
                }
                onClick={() => paymentPlanCsvInputRef.current?.click()}
              >
                Загрузить план поступлений (CSV)
              </button>
              {hasPlanCsv ? (
                <button
                  type="button"
                  className="text-xs font-semibold text-rose-600 hover:text-rose-500"
                  onClick={() => void clearPaymentPlanCsv()}
                >
                  Сбросить план (CSV)
                </button>
              ) : null}
            ...
            {paymentPlanHydrated ? (
              <div className={`grid gap-1 text-[11px] ${presDark ? "text-slate-300" : "text-slate-700"}`}>
                <div>
                  <span className={presDark ? "text-slate-500" : "text-slate-500"}>План: </span>
                  <span className={`font-medium ${presDark ? "text-slate-100" : "text-slate-900"}`}>
                    {schedulePlanMeta?.fileName ?? "не загружен"}
                  </span>
                </div>
                <div>
                  <span className={presDark ? "text-slate-500" : "text-slate-500"}>Факт: </span>
                  <span className={`font-medium ${presDark ? "text-slate-100" : "text-slate-900"}`}>
                    {scheduleFactMeta?.fileName ?? "не загружен"}
                  </span>
                </div>
              </div>
            ) : null}
            {(schedulePlanMeta || scheduleFactMeta) && paymentPlanHydrated ? (
              <div className={`grid gap-1 text-[11px] ${presDark ? "text-slate-400" : "text-slate-600"}`}>
                {schedulePlanMeta ? (
                  <div>
                    <span className={presDark ? "text-slate-500" : "text-slate-500"}>План загружен: </span>
                    <span className={`font-medium tabular-nums ${presDark ? "text-slate-200" : "text-slate-800"}`}>
                      {new Date(schedulePlanMeta.uploadedAt).toLocaleString("ru-RU")}
                    </span>
                    <span className={presDark ? "text-slate-500" : "text-slate-500"}> — </span>
                    <span className={`font-medium ${presDark ? "text-slate-200" : "text-slate-800"}`}>
                      {schedulePlanMeta.uploadedBy}
                    </span>
                  </div>
                ) : null}
                {scheduleFactMeta ? (
                  <div>
                    <span className={presDark ? "text-slate-500" : "text-slate-500"}>Факт загружен: </span>
                    <span className={`font-medium tabular-nums ${presDark ? "text-slate-200" : "text-slate-800"}`}>
                      {new Date(scheduleFactMeta.uploadedAt).toLocaleString("ru-RU")}
                    </span>
                    <span className={presDark ? "text-slate-500" : "text-slate-500"}> — </span>
                    <span className={`font-medium ${presDark ? "text-slate-200" : "text-slate-800"}`}>
                      {scheduleFactMeta.uploadedBy}
                    </span>
                  </div>
                ) : null}
                <div className={presDark ? "text-slate-500" : "text-slate-500"}>
                  Проект (ключ хранилища): <span className="tabular-nums">{paymentPlanProjectId}</span> — общие данные
          ...
          {scheduleWarnings?.length ? (
            <ul
              className={`mt-2 list-inside list-disc text-xs font-medium ${
                presDark ? "text-amber-200/90" : "text-amber-800"
              }`}
            >
