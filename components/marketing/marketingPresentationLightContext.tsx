...
export function MarketingPresentationLightRoot({ children }: { children: ReactNode }) {
  return (
    <MarketingPresentationLightContext.Provider value={true}>
      <div className="theme-marketing-light -mx-3 flex min-h-0 w-[calc(100%+1.5rem)] max-w-none flex-1 flex-col overflow-hidden bg-gradient-to-b from-[#f8fafc] to-[#f3f6fb] px-3 py-2 text-mpl-text antialiased sm:-mx-6 sm:w-[calc(100%+3rem)] sm:px-6 sm:py-4">
        {children}
      </div>
