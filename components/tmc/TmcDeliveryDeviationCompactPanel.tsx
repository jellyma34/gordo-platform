...
import type { TmcDeliveryDeviationSegment } from "@/lib/tmcDeliveryDynamicsAnalytics";
...
function DeviationLegendItem({
  ...
  segment: TmcDeliveryDeviationSegment;
  total: number;
}) {
  return (
...
export function TmcDeliveryDeviationCompactPanel({
  segments,
  factDeliveredCount,
  onTimeOverallPct,
  eligibleCount,
}: {
  segments: TmcDeliveryDeviationSegment[];
  ...
  if (eligibleCount === 0) {
    return (
      <div className="flex min-h-[120px] items-center justify-center px-2 text-center text-xs text-slate-500">
        Нет поставок с определённой плановой датой
      </div>
    );
  }
    ...
    <div className="flex w-full max-w-[250px] flex-col items-center">
          ...
          <PieChart>
            ...
            <Tooltip
              content={({ active, payload }) => {
                ...
                const row = payload[0]?.payload as TmcDeliveryDeviationSegment;
                if (!row) return null;
                return (
                  <div
                  ...
                  >
                    ...
                    <div className="tabular-nums text-slate-300">
                      {row.count} поставок · {formatPct1(row.count, legendTotal)}
                    </div>
                  </div>
                );
              }}
      ...
      {onTimeOverallPct != null ? (
        <div className="mt-2 w-full rounded border border-sky-500/20 bg-sky-500/8 px-1.5 py-1 text-center text-[10px] leading-snug text-sky-200/90">
          {onTimeOverallPct}% поставок выполнены в срок
        </div>
      ) : null}
    </div>
  );
