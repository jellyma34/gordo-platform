...
import type { TmcRequestDeviationSegment } from "@/lib/tmcRequestDynamicsAnalytics";

/**
 * Диаметр donut относительно правой колонки (~250px).
 * Согласован с блоком «Динамика заключения договоров».
 */
...
function DeviationLegendItem({
  ...
  segment: TmcRequestDeviationSegment;
  total: number;
}) {
  return (
...
export function TmcRequestDeviationCompactPanel({
  segments,
  factSubmittedCount,
  onTimeOverallPct,
  eligibleCount,
}: {
  segments: TmcRequestDeviationSegment[];
  factSubmittedCount: number;
  onTimeOverallPct: number | null;
  eligibleCount: number;
}) {
  const gradPrefix = useId().replace(/:/g, "");
  const legendTotal = eligibleCount > 0 ? eligibleCount : segments.reduce((s, x) => s + x.count, 0);
  ...
  if (eligibleCount === 0) {
    return (
      <div className="flex min-h-[120px] items-center justify-center px-2 text-center text-xs text-slate-500">
        Нет заявок с определённой плановой датой
      </div>
    );
  }
    ...
    <div className="flex w-full max-w-[250px] flex-col items-center">
          ...
          <PieChart>
            ...
            <Tooltip
              content={({ active, payload }) => {
                ...
                const row = payload[0]?.payload as TmcRequestDeviationSegment;
                if (!row) return null;
                return (
                  <div
                  ...
                  >
                    ...
                    <div className="tabular-nums text-slate-300">
                      {row.count} заявок · {formatPct1(row.count, legendTotal)}
                    </div>
                  </div>
                );
              }}
            ...
            <Pie
              data={
                chartData.length > 0
                  ? chartData
                  : [{ label: "—", count: 1, color: "#334155", gradId: `${gradPrefix}-empty` }]
              }
              dataKey="count"
              nameKey="label"
              cx="50%"
            ...
            >
              {(chartData.length > 0
                ? chartData
                : [{ gradId: `${gradPrefix}-empty`, color: "#334155" }]
              ).map((entry) => (
                <Cell
                  key={entry.gradId}
                  fill={chartData.length > 0 ? `url(#${entry.gradId})` : entry.color}
                />
              ))}
            </Pie>
          </PieChart>
      ...
      {onTimeOverallPct != null ? (
        <div className="mt-2 w-full rounded border border-sky-500/20 bg-sky-500/8 px-1.5 py-1 text-center text-[10px] leading-snug text-sky-200/90">
          {onTimeOverallPct}% заявок поданы в установленные сроки
        </div>
      ) : null}
    </div>
  );
